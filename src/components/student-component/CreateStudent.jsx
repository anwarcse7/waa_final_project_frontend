...
class CreateStudent extends Component {
    ...
    componentDidMount(){
        ...
        if(this.state.id === '_add'){
            ...
            ApiService.getById(this.state.id).then( (res) =>{
    ...
    saveOrUpdateStudent = (e) => {
        e.preventDefault();
        let student = {firstName: this.state.firstName, lastName: this.state.lastName, email: this.state.email, password: this.state.password};
        console.log('student => ' + JSON.stringify(student));

        // step 5
        if(this.state.id === '_add'){
            ApiService.create(student).then(res =>{
                this.props.history.push('/students');
            });
        }else{
            ApiService.update(student, this.state.id).then( res => {
                this.props.history.push('/students');
            });
        }
    }
